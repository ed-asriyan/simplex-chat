...
fun stripVideoMetadata(inputPath: String, outputPath: String) {
  ...
  fun processContainer(d: ByteArray, offset: Int, end: Int, depth: Int = 0): ByteArray {
    ...
    while (pos + 8 <= end) {
      ...
      } else if (boxType == "trak" && isTimedMetadataTrack(d, pos + 8, pos + boxSize)) {
        // Timed metadata track (GPS telemetry from Pixel/GoPro/DJI/360 cameras) —
        // replace entire trak with "free" to strip the GPS breadcrumb trail
        containerOut.write(writeBE32(boxSize))
        containerOut.write("free".toByteArray(Charsets.US_ASCII))
        containerOut.write(ByteArray(boxSize - 8))
